...
// Gets input from user on submit and calls getData
	$('.search-form').submit(function(e) {
		e.preventDefault();
		user_input = $('#search-box').val();
		getData({q: user_input}, renderView);
...
// Checks if gif clicked is not the champion        
		if ($(this).attr('src') !== gifArray[championIndex]) {
//Sets the clicked gif as the new champion             
			championIndex = challengerIndex;
			updateUrls();
		}
// loads next challenger after every click
		challengerIndex++;
		updateUrls();
// makes another call to api at the last index of the array        
		if(challengerIndex === gifArray.length) {
			getData({q: user_input, offset: currentOffset}, renderView);
		}
		updateSrcs();
